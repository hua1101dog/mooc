...
const polyline = new AirocovMap.Markers.PolyLine();
const vidicon = new AirocovMap.Markers.Vidicon();
const addWindow = (lnglat, map, name, str, height) => {
  ...
  let infowindow = new AirocovMap.Controls.InfoWindow({
    //信息窗内容，是一个dom
    content: str,
...
const addRail = ({ points, map }) => {
  const polylineOvu = new OvuMap.Markers.PolyLine();
  console.log('polylineOvu :', polylineOvu);
};
const addTolauers = ({ points, map }) => {
  //创建画线类
  var PolyLineOvu = new OvuMap.Markers.PolyLine();
  console.log(' :', );
  //线段的坐标
  //线段的配置
  var config = {
  ...
  //创建线段
  var line = PolyLineOvu.drawLine(points, config);
  //添加到对应楼层中图层
  var lineId = map.addToLayer(line, "cytd", "testlayer2", true);
};
...
export default {
  ...
  mounted() {
    if (this.config) AirocovMap.Config.set(this.config);
    ...
    setMap(mapList) {
      ...
      if (this.map) {
        ...
        this.map.event.on("loaded", e => {
          this.map.control.enableRotate = false;
          this.map.addFences = fences => {
            addFences(fences, this.map, mapList[0].name);
          };
          this.map.addMarker = (
            lnglat,
            imgUrl,
