...
(function() {
    ...
    U = X;
    if ($.$crossFrame) { try { while (U.parent && U.parent[M] != U[M] && U.parent[M][C]("frameset").length == 0) U = U.parent } catch (N) {} }
    if (!U.$dp) U.$dp = { ff: G, ie: R, opera: a, status: 0, defMinDate: $.minDate, defMaxDate: $.maxDate };
    B();
    if ($.$preLoad && $dp.status == 0) E(X, "onload", function() { T(null, true) });
    if (!X[M].docMD) {
        E(X[M], "onmousedown", D);
        X[M].docMD = true
    }
        ...
        obj = {
            ...
            show: function() {
                var A = U[M].getElementsByTagName("div"),
                    $ = 100000;
                for (var B = 0; B < A.length; B++) { var _ = parseInt(A[B].style.zIndex); if (_ > $) $ = _ }
    ...
    function T(N, F) {
        ...
        if (F) {
            ...
            if (!N.el || N.el["My97Mark"] === true || N.el.disabled || ($dp.dd && O($dp.dd) != "none" && $dp.dd.style.left != "-970px")) { try { N.el["My97Mark"] = false } catch (C) {} return }
            I(N);
        ...
        function L() { if (R && U != X && U[M].readyState != "complete") return false; return true }
    ...
    function Q(_, $) { return _.currentStyle ? _.currentStyle[$] : document.defaultView.getComputedStyle(_, false)[$] }
    ...
    function I(G, _) {
        ...
        if (!$dp.dd || G.eCont || ($dp.dd && (G.getRealLang().name != $dp.dd.lang || G.skin != $dp.dd.skin))) {
            if (G.eCont) E(G.eCont, G);
                ...
                // U[M].querySelector('#routeBody').appendChild($dp.dd);
                // console.log('asdfafasfasdf');
                // U[M].querySelector('[uib-modal-window][index="0"]>.modal-dialog>.modal-content').appendChild($dp.dd);
                E($dp.dd, G);
                ...
                // 滚动页面 隐藏datepicker
                // begin wjlong 模态框里 my97 datepicker 不随页面滚动 bug: 813  这里是第一部分 第二部分在ui-bootstrap-tpls.js（../angular1.5.11/ui-bootstrap-tpls.js）源码里
                document.querySelector('.view_body').onscroll = function() {
                    var datepicker = document.querySelector('[lang="zh-cn"]');
        ...
        function B(I) {
            ...
            var H = V(C),
                ...
                _ = $dp.dd.offsetWidth;
            if (isNaN(B)) B = 0;
            if (($.topM + H.bottom + E > D.height) && ($.topM + H.top - E > 0)) B += A.top + $.topM + H.top - E - 2;
